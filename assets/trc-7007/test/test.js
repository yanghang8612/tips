...
describe("TRC7007Zkml.sol", function () {

    async function deployTRC7007Fixture() {
        const verifier = await deployVerifierFixture();

        const TRC7007 = await ethers.getContractFactory("TRC7007Zkml");
        const erc7007 = await TRC7007.deploy("testing", "TEST", verifier.address);
    ...
    describe("mint", function () {
        it("should mint a token", async function () {
            const erc7007 = await deployTRC7007Fixture();
        ...
        it("should not mint a token with invalid proof", async function () {
            const erc7007 = await deployTRC7007Fixture();
            const [owner] = await ethers.getSigners();
            await expect(erc7007.mint(owner.address, prompt, aigcData, uri, invalidProof)).to.be.revertedWith("TRC7007: invalid proof");
        });
        ...
        it("should not mint a token with same data twice", async function () {
            ...
            const [owner] = await ethers.getSigners();
            await erc7007.mint(owner.address, prompt, aigcData, uri, validProof);
            await expect(erc7007.mint(owner.address, prompt, aigcData, uri, validProof)).to.be.revertedWith("TRC721: token already minted");
        });
...
describe("TRC7007Enumerable.sol", function () {

    async function deployTRC7007EnumerableFixture() {
        const verifier = await deployVerifierFixture();
        const [owner] = await ethers.getSigners();

        const TRC7007Enumerable = await ethers.getContractFactory("MockTRC7007Enumerable");
        const erc7007Enumerable = await TRC7007Enumerable.deploy("testing", "TEST", verifier.address);
    ...
    it("should return token id by prompt", async function () {
        const erc7007Enumerable = await deployTRC7007EnumerableFixture();
...
describe("TRC7007Opml.sol", function () {

    async function deployTRC7007Fixture() {
        const opmlLib = await deployOpmlLibFixture();

        const TRC7007 = await ethers.getContractFactory("TRC7007Opml");
        const erc7007 = await TRC7007.deploy("testing", "TEST", opmlLib.address);
        await erc7007.deployed();
        return erc7007;
    }
    ...
    describe("mint", function () {
        ...
        it("should not mint a token with same data twice", async function () {
            ...
            const [owner] = await ethers.getSigners();
            await erc7007.mint(owner.address, prompt, aigcData, uri, 0x00);
            await expect(erc7007.mint(owner.address, prompt, aigcData, uri, 0x00)).to.be.revertedWith("TRC721: token already minted");
        });
