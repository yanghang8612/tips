...
import "./ITRC7007Updatable.sol";
import "./IOpmlLib.sol";

/**
 * @dev Implementation of the {ITRC7007} interface.
 */
contract TRC7007Opml is TRC165, ITRC7007Updatable, TRC721URIStorage {
    ...
    function addAigcData(
        ...
        require(tokenIdToRequestId[tokenId] == 0, "TRC7007: requestId already exists");
    ...
    /**
     * @dev See {ITRC7007Updatable-update}.
     */
    function update(
        bytes calldata prompt,
        bytes calldata aigcData
    ) public virtual override {
        require(verify(prompt, aigcData, prompt), "TRC7007: invalid aigcData"); // proof argument is not used in verify() function for opML, so we can pass prompt as proof
        uint256 tokenId = uint256(keccak256(prompt));
        require(ownerOf(tokenId) != address(0), "TRC7007: nonexistent token");
